# RefactoringKata
Uplus x AWS 리팩토링을 연습할 수 있는 예제 입니다.
...
- 판매하는 나머지 일수가 0보다 작으면, `Quality` 값은 **2배**로 움직입니다.(떨어지거나 올라갑니다.).
- `Quality` 값은 결코 음수가 되지는 않습니다.
- "**Aged Brie**"(오래된 브리치즈)은(는) 시간이 지날수록 `Quality` 값이 올라갑니다.
- `Quality` 값은 50를 초과 할 수 없습니다.
- Sulfuras는 전설의 아이템이므로, 반드시 판매될 필요도 없고(sellIn이 변하지 않음) Quality 값도 떨어지지 않습니다.
